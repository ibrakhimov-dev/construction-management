...
import { base_url, edit_house_expenses_api_url, current_house_expenses_api_url } from '../API/baseURL';
...
function EditDailyExpenses() {
    ...
    useEffect(() => {
        axios.get(current_house_expenses_api_url(location.state.id), {headers})
        .then((res) => {
    ...
    function editHouseExpenses () {
        axios.put(edit_house_expenses_api_url(location.state.id), {summa: summa, date: `${date.$y}-${correctDate(date.$M+ 1)}-${date.$D}`, comment: comment,}, {headers})
        .then((res) => {
